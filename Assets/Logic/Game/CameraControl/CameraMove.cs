...
public class CameraMove: MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!UnitControl.isSelect && !NoMove)
        {
            EdgeMove();
        }
        if (OverallHorizontal != Screen.width)
        {
            EdgeInit();
        }
        if (OverallVertical != Screen.height)
        {
            EdgeInit();
        }
    }
    ...
    {
        OverallHorizontal = Screen.width;
        OverallVertical = Screen.height;
        HorizontalA = OverallHorizontal / 5;
        HorizontalB = HorizontalA * 4;
        VerticalA = OverallVertical / 5;
        VerticalB = VerticalA * 4;
    }
    ...
    private void EdgeMove()
    {
        ...
        Quaternion cameraRotation = transform.rotation;
        // 获取摄像机的旋转向量
        Vector3 forward = cameraRotation * Vector3.forward; // 摄像机正前方
        Vector3 right = cameraRotation * Vector3.right; // 摄像机右方
        Vector3 up = cameraRotation * Vector3.up; // 摄像机上方
        if (Input.mousePosition.x < 0)
        {
            if (Input.mousePosition.y > VerticalA && Input.mousePosition.y < VerticalB)
            {
                transform.Translate(-right * moveSpeed * Time.deltaTime, Space.World);
                // 左
            }
            else if (Input.mousePosition.y < VerticalA)
            {
                transform.Translate((-right + -up) * moveSpeed * Time.deltaTime, Space.World);
                // 左下
            }
            else if (Input.mousePosition.y > VerticalB)
            {
                transform.Translate((-right + up) * moveSpeed * Time.deltaTime, Space.World);
                // 左上
            }
        }
        else if (Input.mousePosition.x > OverallHorizontal)
        {
            if (Input.mousePosition.y > VerticalA && Input.mousePosition.y < VerticalB)
            {
                transform.Translate(right * moveSpeed * Time.deltaTime, Space.World);
        ...
        else if (Input.mousePosition.y > OverallVertical)
        {
            if (Input.mousePosition.x > HorizontalA && Input.mousePosition.x < HorizontalB)
            {
                transform.Translate(up * moveSpeed * Time.deltaTime, Space.World);
                // 上
            }
            else if (Input.mousePosition.x < HorizontalA)
            {
                transform.Translate((-right + up) * moveSpeed * Time.deltaTime, Space.World);
                // 左上
            }
            else if (Input.mousePosition.x > HorizontalB)
            {
                transform.Translate((right + up) * moveSpeed * Time.deltaTime, Space.World);
                // 右上
            }
